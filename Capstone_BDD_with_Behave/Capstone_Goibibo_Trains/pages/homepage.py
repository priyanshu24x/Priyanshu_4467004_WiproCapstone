...
class HomePage(BasePage):
    ...
    AUTO_SUGGEST_OPTIONS = (By.XPATH,
                            "//div[contains(@class, 'styles_FswAutoCompItemDesc')] | //p[contains(@class, 'styles_FswAutoCompItemDescTitle')]")
    CALENDAR_NEXT_ARROW = (
        By.XPATH,
    ...
    SELECTED_SOURCE_VAL = (By.XPATH,
                           "//p[contains(@class, 'styles_FswFldHeading') and text()='From']/following-sibling::p[1]")
    SELECTED_DESTINATION_VAL = (By.XPATH,
                                "//p[contains(@class, 'styles_FswFldHeading') and text()='To']/following-sibling::p[1]")
    ...
    def enter_source(self, source_station_code):
        ...
        self.wait.until(
            lambda driver: len(driver.find_elements(*self.AUTO_SUGGEST_OPTIONS)) > 0
                           and source_station_code.lower() in driver.find_elements(*self.AUTO_SUGGEST_OPTIONS)[
                               0].text.lower())
    ...
    def enter_destination(self, destination_station_code):
        ...
        self.wait.until(
            lambda driver: len(driver.find_elements(*self.AUTO_SUGGEST_OPTIONS)) > 0
                           and destination_station_code.lower() in driver.find_elements(*self.AUTO_SUGGEST_OPTIONS)[
                               0].text.lower())
